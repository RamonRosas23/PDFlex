...
class PdfCompareEngineTests(unittest.TestCase):
    def test_detects_visual_and_text_changes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with PdfRenderDocument(output) as report:
                self.assertGreaterEqual(report.page_count, 2)
                self.assertIn("Reporte de comparacion PDF", report.extract_text(0))
    ...
    def test_identical_pdfs_create_clean_report_without_changes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with PdfRenderDocument(output) as report:
                ...
                self.assertIn("Sin diferencias", report.extract_text(0))
    ...
    def _make_pdf(path: Path, pages: list[str]) -> Path:
        canvas = Canvas(str(path), pagesize=(360, 220))
        for text in pages:
            canvas.setFont("Helvetica", 18)
            canvas.drawString(36, 146, text)
            canvas.showPage()
        canvas.save()
        return path
