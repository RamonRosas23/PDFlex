...
4. **Espacio de inserción PyMuPDF:** *(corregido por sonda empírica del 2026-06-09, PyMuPDF 1.27.2.3 — Fase 0 Task 3)* las APIs `insert_textbox`/`draw_*`/`insert_image` interpretan rectángulos **en espacio display** (rotación ya aplicada): la conversión es identidad, y `rotate=0` deja el texto horizontal en pantalla en cualquier `/Rotate`. La receta antigua "`rect × derotation_matrix` + `rotate=page.rotation`" (era válida en versiones previas y es el origen del bug del membrete) hoy PRODUCE la transposición. La conversión sigue encapsulada en `display_rect_to_insertion()` como punto único de control: si PyMuPDF vuelve a cambiar el contrato, la prueba reina falla y el fix vive en un solo lugar. `show_pdf_page` se verifica por separado (membrete documentó que ignora `/Rotate`).
...
- Toda inserción pasa por `display_rect_to_insertion` (chokepoint; identidad en PyMuPDF ≥1.27 — ver §12.4).
- El texto queda **visualmente** con el ángulo que el usuario fijó sin compensación manual: en PyMuPDF 1.27 `rotate=` e inserciones operan en espacio display (`rotate=0` = horizontal en pantalla en cualquier `/Rotate`); el ángulo libre va en `morph=(centro, Matrix(ángulo))`, con semántica fijada empíricamente por los tests de Task 5.
