...
- Draft customer-facing commercial documents now exist: `EULA.md`,
  `PRIVACY_NOTICE.md`, `THIRD_PARTY_NOTICES.md`, `RELEASE_CHECKLIST.md` and
  `CODE_SIGNING.md`.
- The test suite passes with the dependency guard.

## Remaining commercial blockers

1. **Legal documents are drafts.** `EULA.md` and `PRIVACY_NOTICE.md` must be
   completed with real company data, support/privacy emails, governing law,
   venue, license limits and sale-channel terms, then reviewed by counsel.
...
- [ ] Confirm `dist\PDFlex\legal\EULA.md` exists.
- [ ] Confirm `dist\PDFlex\legal\PRIVACY_NOTICE.md` exists.
...
- [ ] Complete placeholders and review EULA/privacy/support text for the exact
      sale channel.
